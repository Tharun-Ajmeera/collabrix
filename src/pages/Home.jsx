...
function FloatingOrbs() {
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none" }}>
      <div style={{ position: "absolute", width: 600, height: 600, borderRadius: "50%", background: "radial-gradient(circle, rgba(83,64,200,0.18) 0%, transparent 70%)", top: "-150px", left: "50%", transform: "translateX(-50%)", animation: "orb1 8s ease-in-out infinite" }} />
      <div style={{ position: "absolute", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, rgba(139,124,246,0.12) 0%, transparent 70%)", top: "200px", right: "-100px", animation: "orb2 10s ease-in-out infinite" }} />
      <div style={{ position: "absolute", width: 300, height: 300, borderRadius: "50%", background: "radial-gradient(circle, rgba(83,64,200,0.1) 0%, transparent 70%)", top: "300px", left: "-80px", animation: "orb3 12s ease-in-out infinite" }} />
...
function Hero() {
  ...
  return (
    <section style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "120px 24px 80px", textAlign: "center", position: "relative" }}>
      <FloatingOrbs />
      <div style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(20px)", transition: "all 0.7s ease", transitionDelay: "0.1s", display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 999, marginBottom: 28, background: "rgba(83,64,200,0.15)", border: "1px solid rgba(139,124,246,0.3)", fontSize: 12, color: "#8B7CF6", fontWeight: 500 }}>
      ...
      <h1 style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(24px)", transition: "all 0.8s ease", transitionDelay: "0.2s", fontSize: "clamp(42px, 7vw, 80px)", fontWeight: 600, lineHeight: 1.08, letterSpacing: "-2px", color: "#fff", margin: "0 0 12px" }}>
        Discover. Connect.
      </h1>
      <h1 style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(24px)", transition: "all 0.8s ease", transitionDelay: "0.3s", fontSize: "clamp(42px, 7vw, 80px)", fontWeight: 600, lineHeight: 1.08, letterSpacing: "-2px", background: "linear-gradient(135deg, #8B7CF6, #5340C8, #A899F0)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", margin: "0 0 28px" }}>
        Compete.
      </h1>
      <p style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(20px)", transition: "all 0.8s ease", transitionDelay: "0.4s", fontSize: 17, color: "rgba(255,255,255,0.45)", maxWidth: 520, lineHeight: 1.75, margin: "0 0 40px" }}>
        Find hackathons, build your skill profile, form teams across India, and grow through pure knowledge reels — all in one place.
      </p>
      <div style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(20px)", transition: "all 0.8s ease", transitionDelay: "0.5s", display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center" }}>
        <a href="#signup" style={{ padding: "13px 28px", borderRadius: 999, fontSize: 14, fontWeight: 500, color: "#fff", textDecoration: "none", background: "linear-gradient(135deg, #5340C8, #7B6EE0)", border: "1px solid rgba(139,124,246,0.5)", transition: "transform 0.2s, box-shadow 0.2s", boxShadow: "0 0 30px rgba(83,64,200,0.3)" }}
        ...
        >Get early access →</a>
        <a href="#features" style={{ padding: "13px 28px", borderRadius: 999, fontSize: 14, fontWeight: 500, color: "rgba(255,255,255,0.6)", textDecoration: "none", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", transition: "all 0.2s" }}
          onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,255,255,0.08)"; e.currentTarget.style.color = "#fff"; }}
          onMouseLeave={e => { e.currentTarget.style.background = "rgba(255,255,255,0.05)"; e.currentTarget.style.color = "rgba(255,255,255,0.6)"; }}
        >See how it works</a>
      </div>
      <div style={{ opacity: visible ? 1 : 0, transition: "opacity 0.8s ease", transitionDelay: "0.7s", display: "flex", gap: 32, marginTop: 48, flexWrap: "wrap", justifyContent: "center" }}>
...
function FeatureCard({ feature, index }) {
  ...
  return (
    <div ref={ref} style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(30px)", padding: "28px", borderRadius: 16, background: hovered ? "rgba(83,64,200,0.12)" : "rgba(255,255,255,0.03)", border: hovered ? "1px solid rgba(139,124,246,0.4)" : "1px solid rgba(255,255,255,0.07)", cursor: "default", transition: `opacity 0.6s ease ${index * 0.08}s, transform 0.6s ease ${index * 0.08}s, background 0.3s, border 0.3s` }}
...
function Features() {
  ...
  return (
    <section id="features" style={{ padding: "80px 24px", maxWidth: 1100, margin: "0 auto" }}>
      <div ref={ref} style={{ textAlign: "center", marginBottom: 56, opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(20px)", transition: "all 0.7s ease" }}>
...
function HowItWorks() {
  ...
  return (
    <section style={{ padding: "80px 24px", position: "relative" }}>
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, transparent, rgba(83,64,200,0.05), transparent)", pointerEvents: "none" }} />
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 2 }}>
          {steps.map((s, i) => (
            <div key={s.num} style={{ padding: "28px 24px", background: "rgba(255,255,255,0.02)", borderTop: "1px solid rgba(255,255,255,0.07)", borderBottom: "1px solid rgba(255,255,255,0.07)", borderLeft: i === 0 ? "1px solid rgba(255,255,255,0.07)" : "none", borderRight: "1px solid rgba(255,255,255,0.07)" }}>
...
function Signup() {
  ...
  return (
    <section id="signup" style={{ padding: "100px 24px", textAlign: "center" }}>
      <div ref={ref} style={{ maxWidth: 560, margin: "0 auto", opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(30px)", transition: "all 0.8s ease" }}>
        <h2 style={{ fontSize: "clamp(28px, 4vw, 44px)", fontWeight: 600, color: "#fff", letterSpacing: "-1px", margin: "0 0 16px" }}>Be the first to join</h2>
        ...
        {submitted ? (
          <div style={{ padding: "16px 24px", borderRadius: 12, fontSize: 14, fontWeight: 500, background: "rgba(83,64,200,0.2)", border: "1px solid rgba(139,124,246,0.4)", color: "#A899F0" }}>
            You're on the list! We'll notify you when Collabrix India launches 🎉
          </div>
        ) : (
          <div style={{ display: "flex", gap: 10, maxWidth: 440, margin: "0 auto", padding: "6px 6px 6px 18px", borderRadius: 999, background: "rgba(255,255,255,0.04)", border: focused ? "1px solid rgba(139,124,246,0.6)" : "1px solid rgba(255,255,255,0.1)", transition: "border 0.2s" }}>
            <input type="email" placeholder="Enter your college email" value={email} onChange={e => setEmail(e.target.value)} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} onKeyDown={e => e.key === "Enter" && email.includes("@") && setSubmitted(true)}
              style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontSize: 14, color: "#fff", minWidth: 0 }} />
            <button onClick={() => email.includes("@") && setSubmitted(true)} style={{ padding: "10px 20px", borderRadius: 999, fontSize: 13, fontWeight: 500, color: "#fff", border: "none", cursor: "pointer", background: "linear-gradient(135deg, #5340C8, #7B6EE0)", whiteSpace: "nowrap" }}>
              Get early access
            </button>
          </div>
        )}

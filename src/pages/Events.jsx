...
const EVENTS = [
  { id: 1, name: "Smart India Hackathon 2026", org: "Government of India", domain: "Open Innovation", date: "June 15, 2026", location: "Pan India", prize: "₹1,00,000", mode: "Offline", tag: "Hackathon", color: "#5340C8" },
  { id: 2, name: "HackWithInfy 2026", org: "Infosys", domain: "AI / ML", date: "July 10, 2026", location: "Bangalore", prize: "₹50,000", mode: "Hybrid", tag: "Hackathon", color: "#1D9E75" },
  { id: 3, name: "Google Solution Challenge", org: "Google", domain: "Social Impact", date: "Aug 5, 2026", location: "Online", prize: "Global Recognition", mode: "Online", tag: "Challenge", color: "#E24B4A" },
  { id: 4, name: "Flipkart GRiD 6.0", org: "Flipkart", domain: "E-Commerce Tech", date: "Aug 20, 2026", location: "Online", prize: "₹75,000", mode: "Online", tag: "Challenge", color: "#EF9F27" },
  { id: 5, name: "HackCBS 9.0", org: "Shaheed Sukhdev College", domain: "Open Theme", date: "Sep 12, 2026", location: "Delhi", prize: "₹30,000", mode: "Offline", tag: "Hackathon", color: "#5340C8" },
  { id: 6, name: "ETHIndia 2026", org: "Devfolio", domain: "Web3 / Blockchain", date: "Oct 1, 2026", location: "Bangalore", prize: "$50,000", mode: "Offline", tag: "Hackathon", color: "#7B6EE0" },
  { id: 7, name: "CodeChef SnackDown", org: "CodeChef", domain: "Competitive Coding", date: "Oct 15, 2026", location: "Online", prize: "₹5,00,000", mode: "Online", tag: "Contest", color: "#1D9E75" },
  { id: 8, name: "Microsoft Imagine Cup", org: "Microsoft", domain: "AI / Tech for Good", date: "Nov 3, 2026", location: "Online", prize: "$100,000", mode: "Online", tag: "Challenge", color: "#185FA5" },
  { id: 9, name: "Myntra HackerRamp", org: "Myntra", domain: "Fashion Tech", date: "Nov 20, 2026", location: "Bangalore", prize: "₹40,000", mode: "Hybrid", tag: "Hackathon", color: "#D4537E" },
];
    ...
    <div style={{ minHeight: "100vh", background: "#08080C", fontFamily: "'Inter', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .event-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07); border-radius: 16px; padding: 24px; transition: all 0.25s; }
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "80px 24px 60px" }}>
        <div style={{ paddingTop: 40, marginBottom: 32 }}>
          ...
          <h1 style={{ fontSize: "clamp(28px, 4vw, 48px)", fontWeight: 600, color: "#fff", letterSpacing: "-1px", marginBottom: 10 }}>Hackathons & Events</h1>
          <p style={{ fontSize: 15, color: "rgba(255,255,255,0.4)" }}>{filtered.length} events found across India — find your next competition!</p>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "12px 16px", marginBottom: 20 }}>
          ...
          <input type="text" placeholder="Search events or organisations..." value={search} onChange={e => setSearch(e.target.value)} style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontSize: 14, color: "#fff" }} />
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 32 }}>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center", width: "100%" }}>
            ...
            {MODES.map(m => <button key={m} className={`filter-btn ${mode === m ? "active" : ""}`} onClick={() => setMode(m)}>{m}</button>)}
          </div>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center", width: "100%" }}>
            <span style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", marginRight: 4 }}>Type:</span>
            {TAGS.map(t => <button key={t} className={`filter-btn ${tag === t ? "active" : ""}`} onClick={() => setTag(t)}>{t}</button>)}
          </div>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center", width: "100%" }}>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            ...
            <button onClick={() => { setSearch(""); setDomain("All"); setMode("All"); setTag("All"); }} style={{ marginTop: 16, background: "rgba(83,64,200,0.2)", border: "1px solid rgba(139,124,246,0.3)", color: "#A899F0", padding: "8px 20px", borderRadius: 999, fontSize: 13, cursor: "pointer" }}>Clear all filters</button>
          </div>
        ) : (
              ...
              <div key={event.id} className="event-card">
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 14 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 40, height: 40, borderRadius: 10, background: `${event.color}22`, border: `1px solid ${event.color}44`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>🏆</div>
                    <div>
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)" }}>{event.org}</div>
                      <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 999, fontSize: 10, fontWeight: 500, background: `${event.color}22`, color: event.color, border: `1px solid ${event.color}44`, marginTop: 2 }}>{event.tag}</span>
                    </div>
                  </div>
                  <button onClick={() => toggleSave(event.id)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, color: saved.includes(event.id) ? "#EF9F27" : "rgba(255,255,255,0.2)" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {[["📅 Date", event.date], ["📍 Venue", event.location], ["💡 Domain", event.domain], ["🌐 Mode", event.mode]].map(([label, val]) => (
                    <div key={label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ fontSize: 12, color: "rgba(255,255,255,0.25)", width: 60 }}>{label}</span>
                ...
                <button className="register-btn" style={{ background: `linear-gradient(135deg, ${event.color}, ${event.color}CC)` }}>Register Now →</button>
              </div>
            ))}
          </div>
        )}

...
export default function Inbox() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap => {
      setDms(snap.docs.map(d => ({ id: d.id, type: "dm", ...d.data() })));
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap => {
      setGroups(snap.docs.map(d => ({ id: d.id, type: "group", ...d.data() })));
    });
    return () => unsub();
  }, [user]);
  ...
  // Combine and sort all chats by time
  ...
  const filtered = allChats.filter(chat => {
    const name = chat.type === "dm" ? getOtherUserName(chat) : chat.name;
    return name?.toLowerCase().includes(search.toLowerCase());
  });

  const filteredByTab = filtered.filter(chat => {
    ...
    <div style={{ minHeight: "100vh", background: "#08080C", fontFamily: "'Inter', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tab-btn { background: none; border: none; cursor: pointer; padding: 7px 16px; font-size: 12px; font-weight: 500; border-radius: 999px; transition: all 0.2s; color: rgba(255,255,255,0.4); }
        .tab-btn.active { background: rgba(83,64,200,0.2); color: #A899F0; border: 1px solid rgba(139,124,246,0.3); }
      ...
      <div style={{ paddingTop: 60, paddingBottom: 80 }}>
        ...
        <div style={{ padding: "8px 20px 12px", display: "flex", gap: 6 }}>
          {["all", "dms", "groups"].map(tab => (
            <button key={tab} className={`tab-btn ${activeTab === tab ? "active" : ""}`} onClick={() => setActiveTab(tab)}>
              {tab === "all" ? `All (${allChats.length})` : tab === "dms" ? `💬 DMs (${dms.length})` : `👥 Teams (${groups.length})`}
            </button>
          ))}
        </div>
        ...
        {!loadingChats && allChats.length === 0 && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 24px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 14, color: "rgba(255,255,255,0.4)", maxWidth: 280, marginBottom: 24, lineHeight: 1.6 }}>
              Find students and start chatting or create a team for your next hackathon!
            </div>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", justifyContent: "center" }}>
              <button onClick={() => navigate("/teammates")}
                style={{ padding: "12px 24px", borderRadius: 999, background: "linear-gradient(135deg, #5340C8, #7B6EE0)", color: "#fff", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 500 }}>
                Find Teammates →
              </button>
              <button onClick={() => navigate("/create-team")}
                style={{ padding: "12px 24px", borderRadius: 999, background: "rgba(83,64,200,0.2)", border: "1px solid rgba(139,124,246,0.3)", color: "#A899F0", cursor: "pointer", fontSize: 13, fontWeight: 500 }}>
                + Create Team
              </button>
            </div>
          </div>
        ...
        {/* Chat list */}
        {!loadingChats && filteredByTab.length > 0 && (
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            {filteredByTab.map(chat => {
              ...
              const unread = isDM ? getUnreadCount(chat) : 0;
                ...
                <div key={chat.id} className="chat-row"
                  ...
                  <div style={{ position: "relative", flexShrink: 0 }}>
                    {isDM ? (
                      ...
                      <div style={{ width: 52, height: 52, borderRadius: 14, background: "linear-gradient(135deg, #5340C8, #7B6EE0)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, border: "2px solid rgba(139,124,246,0.3)" }}>
                        {chat.emoji}
                      </div>
                    )}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 3, gap: 8 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
                        ...
                        {!isDM && (
                          <span style={{ fontSize: 9, fontWeight: 600, padding: "2px 6px", borderRadius: 999, background: "rgba(83,64,200,0.25)", color: "#A899F0", border: "1px solid rgba(139,124,246,0.3)", flexShrink: 0 }}>
                            Team
                          </span>
                        )}
                        {!isDM && (
                          <span style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", flexShrink: 0 }}>{chat.members?.length} members</span>
                        )}
                      </div>
                      <span style={{ fontSize: 11, color: unread > 0 ? "#8B7CF6" : "rgba(255,255,255,0.3)", flexShrink: 0 }}>{time}</span>
                    </div>
        ...
        {/* No search results */}
        {!loadingChats && allChats.length > 0 && filteredByTab.length === 0 && (
